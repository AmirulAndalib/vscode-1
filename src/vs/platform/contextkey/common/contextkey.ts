...
export class Parser {
	...
	private _term(): ContextKeyExpression {
		if (this._match(TokenType.Neg)) {
			...
			throw this._errExpected(`KEY, 'true', or 'false'`);
		}
	...
	private _consume(type: TokenType, message: string) {
		...
		throw this._errExpected(message);
	}

	private _errExpected(expected: string) {
		return this._error(`Expected ${expected}.`);
	}
	...
	private _nameToken(token: Token): string {
		...
		switch (token.type) {
			...
			case TokenType.Or:
				return '||';
			default:
				throw illegalState('all other tokens must have a lexeme');
		}
	}
